...
describe("listUserReportsSchema", () => {
  it("aplica defaults de page y limit cuando no vienen", () => {
    const result = listUserReportsSchema.safeParse({ query: {} });
  ...
  it("rechaza page menor a 1", () => {
    const result = listUserReportsSchema.safeParse({ query: { page: "0" } });
    expect(result.success).toBe(false);
  });

  it("rechaza limit mayor a 100", () => {
    const result = listUserReportsSchema.safeParse({ query: { limit: "200" } });
    expect(result.success).toBe(false);
  });

  it("rechaza reportType inválido", () => {
    const result = listUserReportsSchema.safeParse({ query: { reportType: "PEPE" } });
    expect(result.success).toBe(false);
  });

  it("normaliza reportType y animalType a mayúsculas y coacciona los números", () => {
    const result = listUserReportsSchema.safeParse({
      query: { reportType: "lost", animalType: "dog", lat: "-34.6", lng: "-58.3", radiusKm: "5" },
    });
